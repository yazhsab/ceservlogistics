...
import { signedInHome, signInDestination } from "../auth/navigation";
      ...
      <aside className="relative hidden overflow-hidden bg-[#123f36] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative">
          ...
          <p className="mt-5 max-w-lg text-base leading-7 text-emerald-50/75">
            Secure access to your shipments, account information, and the
...
export function LoginPage() {
  ...
  return (
    <AuthFrame
      title="Welcome back"
      description="Sign in to access your shipments and account."
    >
      <form
        className="space-y-5"
...
export function ChangePasswordPage() {
  ...
  const submit = async (values: Values) => {
    try {
      ...
      void navigate(signedInHome(profile), { replace: true });
    } catch (error) {
      setServerError(
        error instanceof Error
          ? error.message
